...
from cfme.infrastructure.datastore import PhysicalServerAllDatastoresView
...
class PhysicalServer(BaseEntity, Updateable, Pretty, PolicyProfileAssignable, WidgetasticTaggable):
    """Model of an i host in cfme.

    Args:
        name: Name of the host.
        hostname: PhysicalServername of the host.
    ...
    class Credential(BaseCredential, Updateable):
        ...
        def __init__(self, **kwargs):
            super(PhysicalServer.Credential, self).__init__(**kwargs)
    ...
    def update(self, updates, validate_credentials=False):
        ...
        if changed:
            ...
            view.flash.assert_success_message(
                'Edit of PhysicalServer / Node "{}" was cancelled by the user'.format(
    ...
    def delete(self, cancel=True):
        ...
        if not cancel:
            view = self.create_view(PhysicalServersView)
            assert view.is_displayed
            view.flash.assert_success_message("The selected PhysicalServers / Nodes was deleted")
    ...
    def is_compliant(self):
        """Check if the PhysicalServer is compliant.
    ...
    def equal_drift_results(self, drift_section, section, *indexes):
        ...
        drift_history_view = self.create_view(PhysicalServerDriftHistory)
        ...
        drift_analysis_view = self.create_view(PhysicalServerDriftAnalysis)
    ...
    def create(self, name, provider, credentials=None, hostname=None, ip_address=None,
        ...
        Args:
           cancel (bool): Whether to cancel out of the creation. The cancel is done after all the
               information present in the PhysicalServer has been filled in the UI.
        ...
        if not cancel:
            ...
            flash_message = 'PhysicalServer / Node " {}" was added'.format(name)
        else:
            view.cancel_button.click()
...
class Discover(CFMENavigateStep):
    VIEW = PhysicalServerDiscoverView
    prerequisite = NavigateToAttribute("parent", "All")
...
def get_credentials_from_config(credential_config_name):
    ...
    return PhysicalServer.Credential(principal=creds["username"], secret=creds["password"])


def get_from_config(provider_config_name):
    """Creates a PhysicalServer object given a yaml entry in cfme_data.
